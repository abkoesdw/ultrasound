...
unique_step = np.unique(cycle_step)
...
feature_temp1 = np.concatenate((cycle_phase, cycle_step, valve_pv220), axis=1)
target_temp1 = np.asarray(ut_pv220).astype(np.int)
...
window_size = 60
target_temp1 = np.concatenate((np.zeros((window_size, 1)), target_temp1))
target_temp = np.empty((2, 0))
target_temp2 = np.zeros((2, 1))
for k in range(window_size, len(target_temp1)):
    target_temp2[0, 0] = np.mean(target_temp1[k - window_size: k + 1, 0])
    target_temp2[1, 0] = np.std(target_temp1[k - window_size: k + 1, 0])
    print(target_temp.shape, target_temp2.shape)
    target_temp = np.concatenate((target_temp, target_temp2), axis=1)
target_temp = target_temp.T
print(feature_temp.shape, target_temp.shape)

list_duration = [2400, 4500, 10800, 23100]
x_train = dict()
y_train = dict()
x_test = dict()
y_test = dict()
